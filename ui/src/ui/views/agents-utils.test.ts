...
describe("agentLogoUrl", () => {
  it("keeps base-mounted control UI logo paths absolute to the mount", () => {
    expect(agentLogoUrl("/ui")).toBe("/ui/BearClawlogo.png");
    expect(agentLogoUrl("/apps/openclaw/")).toBe("/apps/openclaw/BearClawlogo.png");
  });

  it("uses a route-relative fallback before basePath bootstrap finishes", () => {
    expect(agentLogoUrl("")).toBe("BearClawlogo.png");
  });
});
