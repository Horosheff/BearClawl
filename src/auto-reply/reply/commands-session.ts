...
export const handleSessionCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  const discord = channelRuntime.discord;
  if (onDiscord && !discord) {
    return {
      shouldContinue: false,
      reply: { text: "⚠️ Discord is not available (BearClaw: only Telegram)." },
    };
  }

  const discordManager = onDiscord
    ? discord!.threadBindings.getManager(accountId)
    : null;
  ...
  const idleExpiresAt = onDiscord
    ? discord!.threadBindings.resolveInactivityExpiresAt({
        record: discordBinding!,
        defaultIdleTimeoutMs: discordManager!.getIdleTimeoutMs(),
      })
    : idleTimeoutMs > 0
  ...
  const maxAgeMs = onDiscord
    ? discord!.threadBindings.resolveMaxAgeMs({
        record: discordBinding!,
  ...
  const maxAgeExpiresAt = onDiscord
    ? discord!.threadBindings.resolveMaxAgeExpiresAt({
        record: discordBinding!,
        defaultMaxAgeMs: discordManager!.getMaxAgeMs(),
      })
    : maxAgeMs > 0
  ...
  const updatedBindings = (() => {
    if (onDiscord) {
      return action === SESSION_ACTION_IDLE
        ? discord!.threadBindings.setIdleTimeoutBySessionKey({
            targetSessionKey: discordBinding!.targetSessionKey ?? "",
            accountId,
            idleTimeoutMs: durationMs,
          })
        : discord!.threadBindings.setMaxAgeBySessionKey({
            targetSessionKey: discordBinding!.targetSessionKey ?? "",
            accountId,
            maxAgeMs: durationMs,
          });
